...
interface UsaJobsViewProps {
  jobs: JobPosting[];
  tailoredResumes: TailoredResume[];
  onSelectJob: (job: JobPosting) => void;
}

export const UsaJobsView: React.FC<UsaJobsViewProps> = ({
  jobs,
  tailoredResumes,
  onSelectJob,
}) => {
  ...
  const usaJobs = jobs.filter((j) => j.country === 'USA');

  const filtered = usaJobs.filter((j) => {
    if (workModeFilter !== 'All' && !j.work_mode.toLowerCase().includes(workModeFilter.toLowerCase())) {
      return false;
    }
    if (atsFilter !== 'All' && j.ats_platform !== atsFilter) {
      return false;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
            ...
            USA Target Jobs Pipeline
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Targeting US tech markets: Hartford CT, Boston MA, New York NY, Mountain View CA, and US Remote roles.
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs">
          <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-full font-bold border border-indigo-200">
            {usaJobs.length} Active US Postings
          </span>
          <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-full font-bold border border-emerald-200">
            {usaJobs.filter((j) => (j.match_score || 0) >= 60).length} Passed ≥60% Gate
          </span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-1 min-w-[240px]">
          ...
          <input
            type="text"
            placeholder="Search Hartford, Applied Materials, Intuit, Product Owner..."
            value={searchQuery}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((job) => {
          ...
          return (
            <div
              key={job.job_id}
              className="bg-white border border-slate-200 rounded-xl p-5 shadow-2xs hover:border-indigo-300 transition-all flex flex-col justify-between"
            >
              <div className="space-y-3">
                ...
                {/* Compensation */}
                <div className="text-xs font-bold text-indigo-700 bg-indigo-50/60 border border-indigo-100 px-2.5 py-1 rounded inline-block">
                  {job.salary}
                </div>
